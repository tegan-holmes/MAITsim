function [isoStore_delta] = MAITsim_18O(Store_mat,Store_init,Flux_mat,atm_mat,time1,time2)
% Model Agnostic Isotope Tracer simulator v1.0
%   Simulates isotope tracers (18O here) for input storages and fluxes
...
Store1=Store_init(:,1);
isoStore1=Store1.*delta2conc_18O(Store_init(:,2));
isoStore_conc=zeros(storage_count,time_count+1);
isoStore_conc(:,1)=delta2conc_18O(Store_init(:,2));
    ...
    for i=1:storage_count
        ...
        for n=1:K_count(i)
            ...
            if Flux_mat(flux_id,2,t)>0 
                ...
                Inflow_conc=delta2conc_18O(atm_mat(Store_mat(Flux_mat(flux_id,3,t),2,t),3,t));
                %Heavy isotope water added:
                isoInflow(i)=isoInflow(i)+Inflow_conc*Flux_mat(flux_id,1,t)*dt;
            end
        end
        ...
        % If the deviation from mass conservation is over 0.1%, warn
        ...
        if Store_mat(i,1,t)>mass_check_value
            ...
            if Outflow_evap(i)>0
                ...
                [ConcE,delstar]=craig_gordon_18O(isomixonly_conc, atm_mat(Store_mat(i,2,t),1:3,t), E_ET_split);
                ...
                if isoStore_conc(i,t+1)>delta2conc_18O(delstar)
                    if isomixonly_conc>delta2conc_18O(delstar)
                        ...
                        isoStore_conc(i,t+1)=delta2conc_18O(delstar);
                        isoStore2(i)=isoStore_conc(i,t+1)*Store_mat(i,1,t);
                    end
                end
            else
...
isoStore_delta=conc2delta_18O(isoStore_conc);
end

function [ConcE,delstar] = craig_gordon_18O(source_conc,atm,evapFrac)
...
alphastar=exp((-7.685+6.7123*(10^3/Temp_K)-1.6664*(10^6/Temp_K^2)+0.35041*(10^9/Temp_K^3))/1000);
estar=alphastar-1;
...
dell=conc2delta_18O(source_conc); %per mil
...
ConcE=delta2conc_18O(dele);

end

function [conc] = delta2conc_18O(delta)
%Convert delta per mille to concentration for 18O
Rstd=0.0020052; %For internal accuracy using V-SMOW
R=(delta./1000+1)*Rstd;
conc = R./(R+1);
end

function [delta] = conc2delta_18O(conc)
