MAITsim_18O <- function(Store_array,Store_init,Flux_array,atm_array,time1,time2){
    ...
    for(i in 1:storage_count){
      ...
      for(n in 1:K_count[i]){
        ...
        if(Flux_array[flux_id,2,t]>0){
          ...
          Inflow_conc<-delta2conc_18O(atm_array[Store_array[Flux_array[flux_id,3,t],2,t],3,t])
          #Heavy isotope water added:
          isoInflow[i]<-isoInflow[i]+Inflow_conc*Flux_array[flux_id,1,t]*dt
        }
      ...
      if(Store_array[i,1,t]>mass_check_value){
        ...
        if(Outflow_evap[i]>0){
          ...
          E_output<-craig_gordon_18O(isomixonly_conc,atm_array[atm_id,1,t],atm_array[atm_id,2,t],atm_array[atm_id,3,t],E_ET_split)
          ...
          if(isoStore_conc[i,t+1]>delta2conc_18O(E_output[2])){
            if(isomixonly_conc>delta2conc_18O(E_output[2])){
              ...
              isoStore_conc[i,t+1]<-delta2conc_18O(E_output[2])
              isoStore2[i]<-isoStore_conc[i,t+1]*Store_array[i,1,t]
            }
          }
        } else {
  ...
  return(conc2delta_18O(isoStore_conc))
}
craig_gordon_18O <- function(source_conc,Temp_a,relH,del_p,evapFrac){
  ...
  del_l<-conc2delta_18O(source_conc) #per mil
  ...
  concE<-delta2conc_18O(del_e)
  E_output<-c(concE,delstar)
  return(E_output)
}

delta2conc_18O <- function(delta){
